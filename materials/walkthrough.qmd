...
<!--
, often called a latent code or embedding
-->
...
::: {.callout-tip}
**NOTE (IMPORTANT CONCEPT)**: Importantly, in unsupervised machine learning we are not trying to predict anything. For example, say in the data below we can try to predict the number of people who moved to that state last year. This is a **supervised** machine learning problem [@Gareth2017].
:::
<!-- end callout -->
...
Linear regression is a predictive model. PCA is not. You cannot use PCA to predict anything. You can use PCA to pick out patterns in your data.
:::
<!-- end callout -->
...
The principal components are *linear combinations* of the original *features*. Hence they can be a bit difficult to interpret. 
